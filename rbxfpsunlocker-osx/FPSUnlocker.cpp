...
mach_vm_address_t find_task_scheduler() {
    ...
    if (!address) throw std::runtime_error("Couldn't find the task scheduler pattern. Roblox updates can cause this. Check the GitHub for any new updates.");
...
void fps_unlocker::set_fps_cap(double cap) {
    if (roblox_running()) {
        auto task_sheduler = read_memory<mach_vm_address_t>(find_task_scheduler());
        write_memory<double>(task_sheduler + 0x138, (double)(1.0 / cap));
    }
}

void fps_unlocker::init_roblox(double cap, bool unlock) {
    if (unlock) {
        if (!init_roblox_struct()) throw std::runtime_error("Failed to get Roblox process info. Make sure Roblox is open and try again.");
        set_fps_cap(cap);
    }
}

bool fps_unlocker::roblox_running() {
    return get_roblox_process_id() != 0;
}
